...
export class UploaderComponent implements OnInit, AfterViewChecked {
  ...
  constructor(private route: ActivatedRoute, private toastr: ToastrService, private backendService: BackendService, private loadingService: LoadingService, private router: Router, public userData: UserDataService, public s3: S3Service, protected sanitizer: DomSanitizer, private event: EventService, public geo : GEOLocationService, public commonService: CommonService) {
    _this = this;
  }
  ...
  getApproveFormById(id){
    ...
    this.backendService.getsubmittedFormById(id)
    .subscribe(result => {
      ...
      if(this.userRole != Constants.SUPER_ADMIN) {
        while(this.formData.content.includes(Constants.ORGANISATION_EMPTY_PLACEHOLDER)){
          this.formData.content = this.formData.content.replace(Constants.ORGANISATION_EMPTY_PLACEHOLDER, this.userData.getUserData().organization_name.trim());
        }
      }

      /*Load actual content*/
      this.formData.content = this.sanitizer.bypassSecurityTrustHtml(this.formData.content);
  ...
  actions(){
    ...
    if(this.userRole == Constants.SUPER_ADMIN) {
      ...
      $(".add-row-btn").on('click',function(event) {
        ...
        let coloumn = $(this).closest('.question-container').find('table').find("tr:first td").length;
  ...
  onclikedCheckbox(){
    $("input[type='checkbox']").on('click',function(event) {
      ...
      $(this).prop("checked") ? this.setAttribute("checked", "checked") : this.removeAttribute("checked");
    });
  }
  ...
  getTemplateById(id){
    ...
    this.backendService.getSingleTemplateForm(id)
    .subscribe(result => {
      this.loadingService.apiStop();
      if(result.code == 200) {
  ...
  addImageQuestion(){
    ...
    imgContainer.innerHTML = '<div class=\"upload-lbl\">\r\n<div class=\"plus\">+<\/div>\r\n<div class=\"text\">Upload Image<\/div>\r\n<\/div>';
    $(".img-upload-placeholder").on('click',function(event) {
      event.stopImmediatePropagation();
      _this.onClickedSelectImage(this);
    });
  ...
  setImageInUploader(image, context){
    ...
    parent.append('<div class=\"img-container\"><button class=\"remove-img-button\" type=\"button\">\u00D7<\/button>\r\n    \r\n    <img class=\"uploaded-img\" src=\"  ' + image + '  \">\r\n<\/div>');
  ...
  setGeoLocation(lat, lng){
    ...
    _this.geo.getPosition().then(pos=>{
  ...
  addOrganisationPlaceholder(){
    document.execCommand('insertText', false, Constants.ORGANISATION_EMPTY_PLACEHOLDER);
  }

  onClickPreview(){
    this.formData.content = this.sanitizer.bypassSecurityTrustHtml(document.getElementById('formEditor').innerHTML);
    $('#formEditor .answer').attr("contenteditable","true");
    $('#formEditor .answer').attr("data-text","Enter response here");
  }
  ...
  allowTabelDataForAdminOnly(){
    $("#formEditor table td" ).keyup(function() {
      if(this.innerHTML == '' || this.innerHTML == undefined || this.innerHTML == null) {
  ...
  superAdminFormSubmit(form){
    ...
    for(let question of questions){
      if(question.innerHTML == '' || question.innerHTML == undefined || question.innerHTML == null) {
    ...
    this.templateId == undefined ? this.superAdminAddNewForm(form) : this.superAdminEditForm(form);
  }
  ...
  superAdminAddNewForm(form){
    ...
    if(payload.policyid != null && payload.policyid != undefined && payload.policyid != '') {
      payload.policyid = [payload.policyid.endpoint];
    }

    this.loadingService.apiStart();

    this.backendService.templateFormSubmit(payload)
    .subscribe(result => {
      this.loadingService.apiStop();
      if(result.code == 200) {
        this.formData = {};
  ...
  superAdminEditForm(form){
    ...
    this.backendService.templateFormEdit(payload)
    .subscribe(result => {
      this.loadingService.apiStop();
      if(result.code == 200) {
  ...
  submitFilledForm(form){
    ...
    this.backendService.publishMyForm(payload)
    .subscribe(result => {
      this.loadingService.apiStop();
  ...
  editFilledForm(form){
    ...
    this.backendService.editMyForm(payload)
    .subscribe(result => {
      this.loadingService.apiStop();
  ...
  submitReviewFormComment(){
    ...
    this.backendService.submitReviewComment(payload)
    .subscribe(result => {
      this.loadingService.apiStop();
  ...
  approveSubmittedForm(form){
    ...
    this.backendService.approveAssignedForm(payload)
    .subscribe(result => {
      this.loadingService.apiStop();
  ...
  rejectAssignedForm(form){
    ...
    this.backendService.rejectAssignedForm(payload)
    .subscribe(result => {
      this.loadingService.apiStop();
